...
var testPrivatekey = []byte(`-----BEGIN 私钥-----
...
func Test_SetPublicKey(t *testing.T) {
	_, err := NewRSAPublicKey(testPubkey)
	if err != nil {
		t.Error(err)
	}
}
...
func Test_SetPrivateKey(t *testing.T) {
	_, err := NewRSAPrivateKey(testPrivatekey)
	if err != nil {
		t.Error(err)
	}
}
...
func Test_PubENCTYPTPriDECRYPT(t *testing.T) {
	...
	pridecrypt, err := a.PrivateKey().Decrypt(pubenctypt)
	if err != nil {
		t.Error(err)
	}
...
func Test_PriENCTYPTPubDECRYPT(t *testing.T) {
	...
	pubdecrypt, err := a.PublicKey().Decrypt(prienctypt)
	if err != nil {
		t.Error(err)
	}
